...
# Configura globalmente os wrappers `electronXX` de Arch para usar GNOME
# Keyring explicitamente baixo Hyprland, tamén para aplicacións futuras.
update_electron_password_store() {
  replace_file \
...
show_changelog() {
  ...
  info "· As aplicacións dos wrappers de Electron de Arch usan GNOME Keyring."
}

# Instala a configuración global nun único paso comprobable.
apply_update() {
  if ! update_electron_password_store; then
    return 1
  fi
}
